...
impl<T: Data> Widget<T> for Button<T> {
    ...
    fn paint(&mut self, ctx: &mut PaintCtx, data: &T, env: &Env) {
        ...
        let (bg_color, text_color) = if is_active {
            (
                env.get(theme::BUTTON_CLICK_COLOR),
                env.get(theme::BUTTON_CLICK_TEXT_COLOR),
            )
        }
        else if is_hot {
            (
                env.get(theme::BUTTON_ENTER_COLOR),
                env.get(theme::BUTTON_ENTER_TEXT_COLOR),
            )
        }
        else {
            (
                env.get(theme::BUTTON_LEAVE_COLOR),
                env.get(theme::BUTTON_LEAVE_TEXT_COLOR),
            )
        };
        ...
        self.label.set_text_color(text_color);
